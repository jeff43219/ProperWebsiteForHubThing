...
const buildCss = (accent) => `
  ...
  @media(max-width:768px){
    .notes-wrap .notes-sidebar{position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:200;background:rgba(0,0,0,0);transition:transform .25s ease,background .25s ease;transform:translateX(-100%)}
...
export default function NotesLayout({ tag, title, subtitle, accent = "#d4522a", topics = [], reference = [], children }) {
  ...
  return (
    <div className="notes-wrap" style={{ fontFamily: "'Inter', sans-serif", color: "#f0f0f0" }}>
      ...
      {/* Mobile hamburger — fixed, always visible on mobile, shrinks on scroll */}
      <button
      ...
      >
        <svg width={scrolled ? 14 : 18} height={scrolled ? 14 : 18} viewBox="0 0 18 18" fill="none" style={{ transition: "all .2s ease", flexShrink: 0 }}>
